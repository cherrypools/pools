    ...
    var run = function() {
        ...
        for(var i = 0; i < pools.length; i++) {
            ...
            getPoolInfo(pid, poolAddress, function(data, ptr) {
                ...
                var accPerShare = new BigNumber(data.accPerShare).div(1e12).toNumber();
                ...
                var startTime = parseInt($(ptr).attr("data-pool-time"));
                if (now - startTime > 0) {

                    var share = (accPerShare - parseFloat($(ptr).attr("data-pool-share")));
                    if (share > 0) $(ptr).find(".pool-apr-value").html((1 / ((now - startTime) / (365 * 86400)) * share * 100).toFixed(2));
                }
